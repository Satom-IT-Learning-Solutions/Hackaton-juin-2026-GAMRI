...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- GESTION DU CUSTOM SELECT (COURS) & FILTRAGE CLASSE ---
  if (trigger && optionsMenu) {
    const radiosClasse = document.querySelectorAll('input[name="classe"]');
    const optionsCours = optionsMenu.querySelectorAll(".choix");

    // Fonction de filtrage des cours
    const filtrerCoursParClasse = () => {
      const radioCoche = document.querySelector('input[name="classe"]:checked');
      if (!radioCoche) {
        // Si aucune classe n'est cochée, on masque tout par défaut
        optionsCours.forEach((opt) => (opt.style.display = "none"));
        return;
      }

      const classeSelectionnee = radioCoche.value.toLowerCase(); // 'e1', 'e2' ou 'e3'

      optionsCours.forEach((option) => {
        if (option.classList.contains(classeSelectionnee)) {
    ...
    // Écouteur sur les changements de classe
    radiosClasse.forEach((radio) => {
      radio.addEventListener("change", () => {
        // Reset de la sélection du cours actuelle pour éviter les anomalies
        trigger.querySelector("span").innerText = "Sélectionne un cours...";
        if (hiddenInput) hiddenInput.value = "required";

        filtrerCoursParClasse();
      });
    });

    // Exécution initiale au cas où un radio est pré-coché
    filtrerCoursParClasse();
  ...
  if (deployBtn) {
    deployBtn.addEventListener("click", async () => {
      ...
      if (!coursSelectionne || coursSelectionne === "required") {
        if (deployMsg) {
